  ...
  scranQrCode: function (options) {
    ...
    if (options != null && typeof (options.q) != 'undefined') {
      ...
      if (extConfig != null && extConfig.miniUrl && extConfig.miniUrl != 1) {
        ...
        prefix = "https://www.forzadata.cn/wxapp3/coso/" + extConfig.miniUrl + "/";
      } else {
        prefix = "https://www.forzadata.cn/wxapp/coso/";
      }
      if (src.indexOf(prefix) == 0) {
        let qrCode = src.replace(prefix, "");
        let centerId = qrCode.split("/r/studio/")[0];
        let number = qrCode.split("/r/studio/")[1];
        ...
        app.checkAuthToken(function () {
          centerService.signCourse(centerId, number).then(res => {         
            that.setData({
              info: res
            })
            console.log(res)
